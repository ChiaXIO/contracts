...
contract Ownable {
     ...
     */
    modifier onlyOwner() {
        require(_owner == msg.sender, "WARNING: CALLER_IS_NOT_THE_OWNER");
        _;
    }
    ...
    function transferOwnership(address newOwner) public  onlyOwner {
        require(newOwner != address(0), "WARNING: NEW_OWNER_IS_THE_ZERO_ADDRESS");
...
contract Repurchase is Ownable{
    ...
    address public constant Chiax = 0x1093EcdBACa4168F136f3BEfd17a261bfbbeDdA3;
    address public constant Chiax_USDT = 0x94339BbdB9550a5758Da5EC65C547520EC819520;
    ...
    function swap() external onlyCaller returns (uint256 amountOut){
        ...
        (uint256 reserve0, uint256 reserve1,) = IChiaxSwapPair(Chiax_USDT).getReserves();
        uint256 amountInWithFee = amountIn.mul(997);
        amountOut = amountInWithFee.mul(reserve0) / reserve1.mul(1000).add(amountInWithFee);
        _safeTransfer(USDT, Chiax_USDT, amountIn);
        IChiaxSwapPair(Chiax_USDT).swap(amountOut, 0, destroyAddress, new bytes(0));
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'WARNING: TRANSFER_FAILED');
    }
}